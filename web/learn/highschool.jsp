...
//連結區塊資料
Vector<TableRecord> aps = app_sm.selectAll(tblap,"ap_code=? and ap_lang=?",
...
//人流統計用
String setHistory  = IDTool.getUID("highschool", DateTimeTool.dateString(""), 6);
...
function goaction(FORM,JSP){
    ...
    FORM.submit();
                            ...
                            <ul class="school_bar">
                                <%
                                int counter = 0;
                                for(TableRecord cp_record:cps) {%>
                                    <li>
                                    <form name="frm1<%=counter%>">
                                      <input type="hidden" name="cp_id" value="<%= cp_record.getString("cp_id") %>">
                                    </form>
                                    <a href="javascript: goaction(document.forms['frm1<%=counter++%>'],'./highschool.jsp#learnId')" >
                                        <div class="school_barIn">
                                        <%=cp_record.getString("cp_title") %>
                                        </div>
                                    </a>
                                </li>
                                <%} %>
