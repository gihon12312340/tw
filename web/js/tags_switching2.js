...
$(function(){
	var tabQuantity = $(".indexNABg ul.tabs li").length;				//取得頁籤數量
	
	for ( n=1; n<7; n++ ) {
		if ( tabQuantity == n ) {
			$(".indexNABg .img-scroll").addClass("img-scroll"+n);
		}
	}
	...
	if ( tabQuantity==1 ) {									//如果頁籤只有一個
		$(".indexNABg .tabs_btn_now_arrow").hide();
	}
...
$(function(){
	// 預設顯示第一個 Tab
	var _showTab = 0;
	var $defaultLi = $('.indexNABg ul.tabs li span').eq(_showTab).addClass('active');
	//↑原本為var $defaultLi = $('ul.tabs li').eq(_showTab).addClass('active');
	$($defaultLi.attr('href')).siblings().hide();
	
	// 當 li 頁籤被點擊時...
	// 若要改成滑鼠移到 li 頁籤就切換時, 把 click 改成 mouseover
	$('.indexNABg ul.tabs li span').click(function() {
