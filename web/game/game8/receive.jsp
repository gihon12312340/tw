...
<% 
	...
	try {
		...
		if("start".equals(action)) {
			if(!"".equals(uid)) {
				// 更新遊玩人數記錄(記名)
				String setHistory = IDTool.getUID("droneGame", DateTimeTool.dateString(""), 6);
				...
				gs.setValue("gs_no", "droneGame-"+setHistory);
				gs.setValue("gs_account", uid);
				gs.setValue("gs_name", mp.getString("mp_name"));
				gs.setValue("gs_drone_score", "-1");
				gs.setValue("gs_drone_color", "-1");
				gs.setValue("gs_lang", lang);
				gs.setValue("gs_code", "drone");
				gs.setInsert("Web_User");
				app_sm.insert(gs);
			} else {
				// 更新遊玩人數記錄(匿名)
				String setHistory = IDTool.getUID("droneAnon", DateTimeTool.dateString(""), 6);

				// 保存本次遊玩記錄
				TableRecord gs = new TableRecord(tblgs);
				gs.setValue("gs_no", "droneAnon-"+setHistory);
		...
		/*-- 遊玩結束:修正玩家遊玩資訊及遊玩人數數據 --*/
			...
			String gs_drone_score = parser.optString("drone_score");		// 遊玩分數
			String gs_drone_color = parser.optString("drone_color");		// 無人機顏色
			...
			if(!"".equals(uid)) {
				...
				Vector<TableRecord> gss = app_sm.selectAll(tblgs, "gs_code=? and gs_lang=? and gs_account=?"
						, new Object[]{ "drone", lang, uid }, "gs_createdate ASC");
				if(gss.size() == 10) {
				...
				TableRecord gs = app_sm.select(tblgs, "gs_no like ? and gs_account=? and gs_drone_score=? and gs_drone_color=? and gs_code=? and gs_lang=?"
						, new Object[]{ "%droneGame%", uid, "-1", "-1", "drone", lang }, "gs_createdate DESC");
				if(!"".equals(gs.getString("gs_id"))) {
					gs.setValue("gs_drone_score", gs_drone_score);
					gs.setValue("gs_drone_color", gs_drone_color);
				...
				// 更新該玩家本次遊玩記錄(取最新一筆無記錄的資料出來更新)
				TableRecord gs = app_sm.select(tblgs, "gs_no like ? and gs_account=? and gs_drone_score=? and gs_drone_color=? and gs_code=? and gs_lang=?"
						, new Object[]{ "%droneAnon%", "未登入", "-1", "-1", "drone", lang }, "gs_createdate DESC");
				if(!"".equals(gs.getString("gs_id"))) {
					gs.setValue("gs_drone_score", gs_drone_score);
					gs.setValue("gs_drone_color", gs_drone_color);
					gs.setUpdate("Web_User");
