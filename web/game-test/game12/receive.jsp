...
<% 
	...
	try {
		...
		if("start".equals(action)) {
			if(!"".equals(uid)) {
				// 更新遊玩人數記錄(記名)
				String setHistory = IDTool.getUID("forestGame", DateTimeTool.dateString(""), 6);
				...
				gs.setValue("gs_no", "forestGame-"+setHistory);
				gs.setValue("gs_account", uid);
				gs.setValue("gs_name", mp.getString("mp_name"));
				gs.setValue("gs_lang",lang);
				gs.setValue("gs_forest_score1", -1);
				gs.setValue("gs_forest_score2", -1);
				gs.setValue("gs_code", "forest");
				gs.setInsert("Web_User");
				app_sm.insert(gs);
			} else {
				// 更新遊玩人數記錄(匿名)
				String setHistory = IDTool.getUID("forestAnon", DateTimeTool.dateString(""), 6);

				// 保存本次遊玩記錄
				TableRecord gs = new TableRecord(tblgs);
				gs.setValue("gs_no", "forestAnon-"+setHistory);
		...
		/*-- 遊玩結束:修正玩家遊玩資訊及遊玩人數數據 --*/
		    ...
		    String score1 = parser.optString("forest_score_1");
		    String score2 = parser.optString("forest_score_2");
//System.out.println("result:"+score1+";"+score2);
			if(!"".equals(uid)) {
				...
				Vector<TableRecord> gss = app_sm.selectAll(tblgs, "gs_code=? and gs_lang=?  and gs_account=?"
						, new Object[]{ "forest", lang,uid }, "gs_createdate ASC");
...
//System.out.println(mp.getString("mp_id"));
				...
				TableRecord gs = app_sm.select(tblgs, "gs_no like ? and gs_account=? and gs_forest_score1=? and gs_forest_score2=? and gs_code=? and gs_lang=?"
						, new Object[]{ "%forestGame%", uid, "-1","-1","forest", lang }, "gs_createdate DESC");
//System.out.println(gs.getString("gs_id"));
				if(!"".equals(gs.getString("gs_id"))) {
					gs.setValue("gs_forest_score1",Integer.valueOf(score1));
					gs.setValue("gs_forest_score2",Integer.valueOf(score2));
				...
				TableRecord gs = app_sm.select(tblgs, "gs_no like ? and  gs_account=? and gs_code=? and gs_lang=?"
						, new Object[]{ "%forestAnon%", "未登入", "forest", lang }, "gs_createdate DESC");
				if(!"".equals(gs.getString("gs_id"))) {
					gs.setValue("gs_forest_score1",Integer.valueOf(score1));
					gs.setValue("gs_forest_score2",Integer.valueOf(score2));	
					gs.setUpdate("Web_User");
