...
<% 
	...
	try {
		...
		if("start".equals(action)) {
			if(!"".equals(uid)) {
				// 更新遊玩人數記錄(記名)
				String setHistory = IDTool.getUID("dreamGame", DateTimeTool.dateString(""), 6);
				...
				gs.setValue("gs_no", "dreamGame-"+setHistory);
				gs.setValue("gs_account", uid);
				gs.setValue("gs_name", mp.getString("mp_name"));
				gs.setValue("gs_recipe_id", "-1");
				gs.setValue("gs_lang", lang);
				gs.setValue("gs_code", "recipe");
				gs.setInsert("Web_User");
				app_sm.insert(gs);
			} else {
				// 更新遊玩人數記錄(匿名)
				String setHistory = IDTool.getUID("dreamAnon", DateTimeTool.dateString(""), 6);

				// 保存本次遊玩記錄
				TableRecord gs = new TableRecord(tblgs);
				gs.setValue("gs_no", "dreamAnon-"+setHistory);
		...
		/*-- 遊玩結束:修正玩家遊玩資訊及遊玩人數數據 --*/
			...
			String recipe_id = parser.optString("recipe_id");	// 食譜編號
			//System.out.println("uid = " + uid + ", recipe_id = " + recipe_id);

			if(!"".equals(uid)) {
				/*-- 整理該名玩家於本遊戲中的記錄,記錄超過10筆時需將最舊記錄刪除 --*/
				Vector<TableRecord> gss = app_sm.selectAll(tblgs, "gs_code=? and gs_lang=? and gs_account=?"
						, new Object[]{ "recipe", lang, uid }, "gs_createdate ASC");
				if(gss.size() == 10) {
				...
				TableRecord gs = app_sm.select(tblgs, "gs_no like ? and gs_account=? and gs_recipe_id=? and gs_code=? and gs_lang=?"
						, new Object[]{ "%dreamGame%", uid, "-1", "recipe", lang }, "gs_createdate DESC");
				if(!"".equals(gs.getString("gs_id"))) {
					gs.setValue("gs_recipe_id", recipe_id);
				...
				// 更新該玩家本次遊玩記錄(取最新一筆無記錄的資料出來更新)
				TableRecord gs = app_sm.select(tblgs, "gs_no like ? and  gs_account=? and gs_recipe_id=? and gs_code=? and gs_lang=?"
						, new Object[]{ "%dreamAnon%", "未登入", "-1", "recipe", lang }, "gs_createdate DESC");
				if(!"".equals(gs.getString("gs_id"))) {
					gs.setValue("gs_recipe_id", recipe_id);
					gs.setUpdate("Web_User");
					app_sm.update(gs);
