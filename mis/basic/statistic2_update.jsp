...
try{
	...
	if("D".equals(action)){
		...
		if (app_sm.success()) {
			/*
			if(app_sm.selectAll(tblvr, "vr_image=?", new Object[] { vr.getString("vr_image") }).size() == 0) {
				FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+vr.getString("vr_image"));
			}
			*/
			// 回列表頁
	...
	if("S".equals(action)){
		...
		while (i.hasNext()) {
			...
			if (fi.isFormField()){											//這是用來確定是否為文件屬性 
				...
				if("selData".equals(fieldName)){chk_vr = fieldvalue.trim();}
			}
		}
		String list[] = chk_vr.split(",");
		for(int j=0;j<list.length;j++){
			if(!"".equals(list[j].trim())){
				TableRecord vr = app_sm.select(tblvr, "vr_id=?",new Object[] { list[j] });
				vr.setValue("vr_showseq", j);
				vr.setUpdate(app_account);
				app_sm.update(vr);
			}
		}
		app_sm.close();
	...
	//新增
	if("A".equals(action)){		
		TableRecord vr = new TableRecord(tblvr);
		vr.setInsert(app_account);
		//---------------------檔案上傳-----------------------------------------------
		while (i.hasNext()) {
			FileItem fi = (FileItem) i.next();
			if (fi.isFormField()){ //這是用來確定是否為文件屬性 
				...
				if(fieldName.equals("vr_title")){
					Vector checks = app_sm.selectAll(tblvr,"vr_title =? and vr_code=? and vr_lang=?",new Object[]{fieldvalue, code, lang});
					if(checks.size()>0){
						out.println("<script> alert('標題重複!!'); history.back(); </script>");
						return;
					}
				}
				vr.setValue(fieldName, fieldvalue.trim());//設定欄位值
				...
				if (fileName != null && !"".equals(fileName)) {
					...
					*/
					...
					vr.setValue(fi.getFieldName(), fileName_1);//設定圖
				}
			}
		}
		vr.setValue("vr_code", code);	// 識別碼
		vr.setValue("vr_lang", lang);	// 語系		
		app_sm.insert(vr);
		// 回列表頁
		out.write(HtmlCoder.getForm("listpage", code + ".jsp", names, values));
		out.println("<script> alert('新增成功!!');listpage.submit(); </script>");
	}else if("M".equals(action)){//修改
		TableRecord vr = app_sm.select(tblvr, "vr_id=?",new Object[] { vr_id });
		vr.setUpdate(app_account);
		while (i.hasNext()) {
...
//System.out.println("=========");
				...
				if(fieldName.equals("vr_title")){
					Vector checks = app_sm.selectAll(tblvr,"vr_title =? and vr_id <>? and vr_code=? and vr_lang=?",new Object[]{fieldvalue, vr_id, code, lang});
					if(checks.size()>0){
						out.println("<script> alert('標題重複!!'); history.back(); </script>");
						return;
					}else{
				...
				if (fileName != null && !"".equals(fileName)) {
					...
					*/
					...
					if(!"".equals(vr.getString(fi.getFieldName()))) {
						//確認其它資料沒有使用相同檔案
						if(app_sm.selectAll(tblvr, fi.getFieldName()+"=?", new Object[] {vr.getString(fi.getFieldName()) }).size() == 1) {
							FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+vr.getString(fi.getFieldName()));
						}
					}
