	...
	if("A".equals(action)){		
		...
		while (i.hasNext()) {
			...
			if (fi.isFormField()){ //這是用來確定是否為文件屬性 
				...
				String fieldvalue = new String(fi.getString("UTF-8")); //取得值
				if(fieldName.equals("ap_title")){
					Vector checks = app_sm.selectAll(tblap,"ap_title =? and ap_id !=? and ap_code=? and ap_category=?",new Object[]{fieldvalue, ap_id, code,cp_id});
					if(checks.size()>0){
						out.println("<script> alert('標題重複!!'); history.back(); </script>");
						return;
					}
