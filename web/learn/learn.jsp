...
<%
//識別碼
String page_code = "learn";
//網編區資料
TableRecord cp1 = app_sm.select(tblcp,"cp_code=? and cp_lang=?",new Object[]{page_code+"1",lang});
TableRecord cp2 = app_sm.select(tblcp,"cp_code=? and cp_lang=?",new Object[]{page_code+"2",lang});
TableRecord cp3 = app_sm.select(tblcp,"cp_code=? and cp_lang=?",new Object[]{page_code+"3",lang});
//連結區塊資料
Vector<TableRecord> aps = app_sm.selectAll(tblap,"ap_code=? and ap_lang=?",new Object[]{"learn_link",lang},"ap_showseq ASC,ap_createdate DESC");

//人流統計用
String setHistory  = IDTool.getUID("learn", DateTimeTool.dateString(""), 6);
%>
...
<%-- SEO --%>
<%if(!"".equals(cp1.getString("cp_id"))){ %>
<meta name="Robots" content="<%=cp1.getString("cp_robots") %>" />
<meta name="revisit-after" content="<%=cp1.getString("cp_revisit_after") %> days" />
<meta name="keywords" content="<%=cp1.getString("cp_keywords") %>" />
<meta name="copyright" content="<%=cp1.getString("cp_copyright") %>" />
<meta name="description" content="<%=cp1.getString("cp_description") %>" />
<%-- 追蹤碼 --%><%=cp1.getString("cp_seo_track") %>
<%} %>
