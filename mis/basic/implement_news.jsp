...
/*--------------------------------------------------------------------------------------------*/
	...
	// 跳頁參數
	String[] names = new String[] { "npage", "_qtitle","_qstatus" };
	String[] values = new String[] { String.valueOf(pageno), qtitle,qstatus };

	
	if (search_switch) {
		StringBuffer sb = new StringBuffer();
		Vector keys = new Vector();
		sb.append("cp_lang=? and cp_code=? and cp_title like ? and cp_status like?");
		keys.add(lang);
		keys.add(code);
		keys.add("%" + qtitle + "%");
		keys.add("%" + qstatus + "%");
				...
				<table width="95%" border="0" cellspacing="1" cellpadding="0">
					...
					<table width="100%" border="0" align="center" cellpadding="3"
						...
						<tr class="web_table-2-1">
							...
							<td width="65%" align="center">標題名稱</td>
						...
						<%
							for (int i = 0; i < cps.size(); i++) {
								...
								TableRecord cp_post_course = app_sm.select(tblcp,cp.getString("cp_category")); //去抓關聯的發文資料
						%>
						<form name="list<%=i + 1%>" id="list<%=i + 1%>" method="post">
						<tr class="web_table-2-1">
							<td align="center"><%=((pageno - 1) * page_items) + i + 1%></td>
							<td align="center"><%=cp_post_course.getString("cp_title") %></td>
							<td align="center" >
                                <select name="cp_status" onchange="goaction(this.form, '<%=code %>_update.jsp?action=P');">
                                    <option value="N" <%="N".equals(cp.getString("cp_status"))?"selected":"" %>>未審核</option>
                                    <option value="Y" <%="Y".equals(cp.getString("cp_status"))?"selected":"" %>>通過</option>
                                    <option value="R" <%="R".equals(cp.getString("cp_status"))?"selected":"" %>>不通過</option>
                                </select>    
                            </td>						
							<td align="center">
