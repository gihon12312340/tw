...
<% 
	...
	try {
		...
		if("start".equals(action)) {
			if(!"".equals(uid)) {
				// 更新遊玩人數記錄(記名)
				String setHistory = IDTool.getUID("farmingGame", DateTimeTool.dateString(""), 6);
				...
				gs.setValue("gs_no", "farmingGame-"+setHistory);
				gs.setValue("gs_account", uid);
				gs.setValue("gs_name", mp.getString("mp_name"));
				gs.setValue("gs_chicken_score", "-1");
				gs.setValue("gs_lang", lang);
				gs.setValue("gs_code", "chicken");
				gs.setInsert("Web_User");
				app_sm.insert(gs);
			} else {
				// 更新遊玩人數記錄(匿名)
				String setHistory = IDTool.getUID("farmingAnon", DateTimeTool.dateString(""), 6);

				// 保存本次遊玩記錄
				TableRecord gs = new TableRecord(tblgs);
				gs.setValue("gs_no", "farmingAnon-"+setHistory);
		...
		/*-- 遊玩結束:修正玩家遊玩資訊及遊玩人數數據 --*/
			...
			String chicken_score = parser.optString("chicken_score");	// 遊玩分數
			//System.out.println("uid = " + uid + ", chicken_score = " + chicken_score);

			if(!"".equals(uid)) {
				/*-- 整理該名玩家於本遊戲中的記錄,記錄超過10筆時需將最舊記錄刪除 --*/
				Vector<TableRecord> gss = app_sm.selectAll(tblgs, "gs_code=? and gs_lang=? and gs_account=?"
						, new Object[]{ "chicken", lang, uid }, "gs_createdate ASC");
				if(gss.size() == 10) {
				...
				TableRecord gs = app_sm.select(tblgs, "gs_no like ? and gs_account=? and gs_chicken_score=? and gs_code=? and gs_lang=?"
						, new Object[]{ "%farmingGame%", uid, "-1", "chicken", lang }, "gs_createdate DESC");
				if(!"".equals(gs.getString("gs_id"))) {
					gs.setValue("gs_chicken_score", chicken_score);
				...
				// 更新該玩家本次遊玩記錄(取最新一筆無記錄的資料出來更新)
				TableRecord gs = app_sm.select(tblgs, "gs_no like ? and gs_account=? and gs_chicken_score=? and gs_code=? and gs_lang=?"
						, new Object[]{ "%farmingAnon%", "未登入", "-1", "chicken", lang }, "gs_createdate DESC");
				if(!"".equals(gs.getString("gs_id"))) {
					gs.setValue("gs_chicken_score", chicken_score);
					gs.setUpdate("Web_User");
					app_sm.update(gs);
