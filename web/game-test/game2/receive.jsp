...
System.out.println(uid);
		...
		if("start".equals(action)) {
			if(!"".equals(uid)) {
				// 更新遊玩人數記錄(記名)
				String setHistory = IDTool.getUID("aircraftGame", DateTimeTool.dateString(""), 6);
				...
				gs.setValue("gs_no", "aircraftGame-"+setHistory);
				gs.setValue("gs_account", uid);
				gs.setValue("gs_name", mp.getString("mp_name"));
				gs.setValue("gs_lang",lang);
				gs.setValue("gs_aircraft_score", -1);
				gs.setValue("gs_code", "aircraft");
				gs.setInsert("Web_User");
				app_sm.insert(gs);
			} else {
				// 更新遊玩人數記錄(匿名)
				String setHistory = IDTool.getUID("aircraftAnon", DateTimeTool.dateString(""), 6);

				// 保存本次遊玩記錄
				TableRecord gs = new TableRecord(tblgs);
				gs.setValue("gs_no", "aircraftAnon-"+setHistory);
		...
		/*-- 遊玩結束:修正玩家遊玩資訊及遊玩人數數據 --*/
			...
			String aircraft_score = parser.optString("aircraft_score");
System.out.println(aircraft_score);          
			if(!"".equals(uid)) {
				/*-- 整理該名玩家於本遊戲中的記錄,記錄超過10筆時需將最舊記錄刪除 --*/
				Vector<TableRecord> gss = app_sm.selectAll(tblgs, "gs_code=? and gs_lang=? and gs_account=?"
						, new Object[]{ "aircraft", lang, uid }, "gs_createdate ASC");
...
System.out.println(mp.getString("mp_id"));
				...
				TableRecord gs = app_sm.select(tblgs, "gs_no like ? and gs_account=? and gs_aircraft_score=? and gs_code=? and gs_lang=?"
						, new Object[]{ "%aircraftGame%", uid, "-1","aircraft", lang }, "gs_createdate DESC");
System.out.println(gs.getString("gs_id"));
				if(!"".equals(gs.getString("gs_id"))) {
					
					gs.setValue("gs_aircraft_score",Integer.valueOf(aircraft_score));
				...
				// 更新該玩家本次遊玩記錄(取最新一筆無記錄的資料出來更新)
				TableRecord gs = app_sm.select(tblgs, "gs_no like ? and  gs_account=? and gs_aircraft_score=? and gs_code=? and gs_lang=?"
						, new Object[]{ "%aircraftAnon%", "未登入", "recipe","-1", lang }, "gs_createdate DESC");
				if(!"".equals(gs.getString("gs_id"))) {
					gs.setValue("gs_aircraft_score",Integer.valueOf(aircraft_score));
					gs.setUpdate("Web_User");
					app_sm.update(gs);
