...
try{
	...
	if("P".equals(action)) {
		TableRecord cp = app_sm.select(tblcp,cp_id);
		String cp_status = StringTool.validString(request.getParameter("cp_status"));
		cp.setValue("cp_status",cp_status);
	...
	//檔案路徑與設置
	String dir = application.getRealPath("/") + "uploads/"+"post_course"+"/"+lang+"/";
			...
			if (fi.isFormField()){ //這是用來確定是否為文件屬性 
				...
				if(fieldName.equals("cp_title")){
					...
					if(checks.size()>0){
						...
						cp_post_course.setValue(fieldName, fieldvalue.trim());//設定欄位值
					}
				...
				if (fileName != null && !"".equals(fileName)) {
					...
					*/
					...
					cp_post_course.setValue(fi.getFieldName(), fileName_1);//設定圖
				}
			}
		}		
