...
<%	
	// 基本參數
	String code = "activity_information_2"; 					// 模組識別碼
	String show_title = "國資圖-活動資訊維護";			// 模組標題
...
try{
	...
	if("D".equals(action)){
		...
		//FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+ai.getString("ai_image"));
		//FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+ai.getString("ai_mobile"));
		app_sm.delete(ai);
		app_sm.close();
		//確認是否有其它資料顯示相同圖片(有其它檔案欄位再新增code)
		if (app_sm.success()) {
			if(app_sm.selectAll(tblai, "ai_image=?", new Object[] { ai.getString("ai_image") }).size() == 0) {
				FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+ai.getString("ai_image"));
			}
			// 回列表頁
	...
	if("S".equals(action)){
		...
		while (i.hasNext()) {
			...
			if (fi.isFormField()){											//這是用來確定是否為文件屬性 
				...
				if("selData".equals(fieldName)){chk_ai = fieldvalue.trim();}
			}
		}
		String list[] = chk_ai.split(",");
		for(int j=0;j<list.length;j++){
			if(!"".equals(list[j].trim())){
				TableRecord ai = app_sm.select(tblai, "ai_id=?",new Object[] { list[j] });
				ai.setValue("ai_showseq", j);
				ai.setUpdate(app_account);
				app_sm.update(ai);
			}
		}
		app_sm.close();
	...
	//新增
	if("A".equals(action)){		
		TableRecord ai = new TableRecord(tblai);
		ai.setInsert(app_account);
		//---------------------檔案上傳-----------------------------------------------
		while (i.hasNext()) {
			FileItem fi = (FileItem) i.next();
			if (fi.isFormField()){ //這是用來確定是否為文件屬性 
				...
				if(fieldName.equals("ai_title")){
					Vector checks = app_sm.selectAll(tblai,"ai_title =? and ai_code=? and ai_lang=?",new Object[]{fieldvalue, code, lang});
					if(checks.size()>0){
						out.println("<script> alert('標題重複!!'); history.back(); </script>");
						return;
					}
				}
				ai.setValue(fieldName, fieldvalue.trim());//設定欄位值
				...
				//fileName_1 = ai.getString("ai_id")+"_"+fileName_1;
				...
				if (fileName != null && !"".equals(fileName)) {
					...
					*/
					...
					ai.setValue(fi.getFieldName(), fileName_1);//設定圖
				}
			}
		}
		ai.setValue("ai_code", code);	// 識別碼
		ai.setValue("ai_lang", lang);	// 語系		
		app_sm.insert(ai);
		// 回列表頁
		out.write(HtmlCoder.getForm("listpage", code + ".jsp", names, values));
		out.println("<script> alert('新增成功!!');listpage.submit(); </script>");
	}else if("M".equals(action)){//修改
		TableRecord ai = app_sm.select(tblai, "ai_id=?",new Object[] { ai_id });
		ai.setUpdate(app_account);
		while (i.hasNext()) {
			...
			if (fi.isFormField()){ //這是用來確定是否為文件屬性 
				...
				if(fieldName.equals("ai_title")){
					Vector checks = app_sm.selectAll(tblai,"ai_title =? and ai_id <>? and ai_code=? and ai_lang=?",new Object[]{fieldvalue, ai_id, code, lang});
					if(checks.size()>0){
						out.println("<script> alert('標題重複!!'); history.back(); </script>");
						return;
					}else{
				...
				if (fileName != null && !"".equals(fileName)) {
					...
					*/
					...
					if(!"".equals(ai.getString(fi.getFieldName()))){
						//確認其它資料沒有使用相同檔案
						if(app_sm.selectAll(tblai, fi.getFieldName()+"=?", new Object[] {ai.getString(fi.getFieldName()) }).size() == 1) {
							FileTool.deleteFile(app_uploadpath+"/"+code+"/"+lang+"/"+ai.getString(fi.getFieldName()));
						}
					}
