...
<%
	// 基本參數
	String code = "career_exploration_banner"; 					// 模組識別碼
	String show_title = "技職試探常設展輪播";				// 模組標題
...
/*--------------------------------------------------------------------------------------*/		
	Vector aps = app_sm.selectAll(tblap, "ap_code=? and ap_lang=? and ap_category=?", new Object[] { code,lang,dm_id }, "ap_showseq ASC , " + "ap_createdate DESC");
	...
	// 跳頁參數
	String[] names = new String[] { "npage", "_qtitle", "_qemitdate", "_qrestdate" ,"dm_id"};
	String[] values = new String[] { String.valueOf(pageno), qtitle, qemitdate, qrestdate,dm_id };	
	
	if (search_switch) {
		StringBuffer sb = new StringBuffer();
		Vector keys = new Vector();
		sb.append("ap_lang=? and ap_code=? and ap_category=? and ap_title like ?");
		keys.add(lang);
		keys.add(code);
		keys.add(dm_id);
		keys.add("%" + qtitle + "%");
					...
					<table width="100%" border="0" align="center" cellpadding="3"
						...
						<tr>
							<td align="center" colspan="3" class="web_title-1"><%=show_title%>維護&nbsp;&nbsp;
								<%if (add_switch) { %>
								<input type="button" value="新增資料" onclick="javascript:location.href='<%=code%>_a.jsp?dm_id=<%=dm_id %>'" />&nbsp;
								<%} %>
								<%if (list_switch) { %>
								<input type="button" value="回到列表" onclick="javascript:location.href='<%=code%>.jsp?dm_id=<%=dm_id %>'" />&nbsp;
								<%} %>
								<%if (sort_switch) { %>
