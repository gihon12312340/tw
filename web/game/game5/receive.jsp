...
<% 
	...
	try {
		...
		if("start".equals(action)) {
			if(!"".equals(uid)) {
				// 更新遊玩人數記錄(記名)
				String setHistory = IDTool.getUID("paradiseGame", DateTimeTool.dateString(""), 6);
				...
				gs.setValue("gs_no", "paradiseGame-"+setHistory);
				gs.setValue("gs_account", uid);
				gs.setValue("gs_name", mp.getString("mp_name"));
				gs.setValue("gs_image", "-1");
				gs.setValue("gs_paint_score",-1);
				gs.setValue("gs_lang", lang);
				gs.setValue("gs_code", "paint");
				gs.setInsert("Web_User");
				app_sm.insert(gs);
			} else {
				// 更新遊玩人數記錄(匿名)
				String setHistory = IDTool.getUID("paradiseAnon", DateTimeTool.dateString(""), 6);

				// 保存本次遊玩記錄
				TableRecord gs = new TableRecord(tblgs);
				gs.setValue("gs_no", "paradiseAnon-"+setHistory);
		...
		/*-- 遊玩結束:修正玩家遊玩資訊及遊玩人數數據 --*/
			...
			String img = parser.optString("img");				// 彩繪圖片
			String score = parser.optString("paint_score");
			//System.out.println("uid = " + uid + ", img = " + img);
			...
			if(!"".equals(uid)) {
				...
				Vector<TableRecord> gss = app_sm.selectAll(tblgs, "gs_code=? and gs_lang=? and gs_account=?"
						, new Object[]{ "paint", lang, uid }, "gs_createdate ASC");
				if(gss.size() == 10) {
				...
				TableRecord gs = app_sm.select(tblgs, "gs_no like ? and gs_account=? and gs_image=? and gs_code=? and gs_lang=?"
						, new Object[]{ "%paradiseGame%", uid, "-1", "paint", lang }, "gs_createdate DESC");
				if(!"".equals(gs.getString("gs_id"))) {
					if(! "".equals(img)){
					    gs.setValue("gs_image", img);
					}
					if(! "".equals(score)) {
						gs.setValue("gs_paint_score",Integer.valueOf(score));
					}
				...
				// 更新該玩家本次遊玩記錄(取最新一筆無記錄的資料出來更新)
				TableRecord gs = app_sm.select(tblgs, "gs_no like ? and gs_account=? and gs_image=? and gs_code=? and gs_lang=?"
						, new Object[]{ "%paradiseAnon%", "未登入", "-1", "paint", lang }, "gs_createdate DESC");
				if(!"".equals(gs.getString("gs_id"))) {
					gs.setValue("gs_image", "");		// 20210825：匿名遊玩因資料庫讀取問題故改為不保存圖片
					if(! "".equals(score)) {
						gs.setValue("gs_paint_score",score);
					}
					gs.setUpdate("Web_User");
					app_sm.update(gs);
				}
